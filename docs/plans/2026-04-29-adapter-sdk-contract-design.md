...
| 9 | `src/persistence/sdk/mcp/_server.py` | MCP server: lifecycle (`initialize` → `notifications/initialized` → `tools/list`/`tools/call` / `resources/*`); JSON-RPC dispatch; transport stdio (sole `@stable("v0.8")` per ADR-15) + Streamable HTTP (EXPERIMENTAL, gated, NOT v0.8 conformance per ADR-15b — see row 11a + G12) | NEW |
...
**Allowed keywords:** `type`, `properties`, `required`, `additionalProperties` (always `false` at object root), `items`, `minItems`, `maxItems`, `minLength`, `maxLength`, `minimum`, `maximum`, `enum`, `const`, `pattern`, `description`, `default`, `examples` (root-only by generator policy — see § 5.1.1 generator pipeline `preserve_at_root={"examples"}`; the keyword is profile-allowed semantically, but the generator strips non-root occurrences to keep schema bytes minimal and review-friendly). `format` allowed for `uuid` / `date-time` / `uri` only.
