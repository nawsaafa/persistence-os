...
| **2.3c.1** | Skill library coder integration — `:skill/define` + `:skill/lookup` audit-anchored ops; closure-over-injected-`SkillLibrary` singleton (LD1 R0-fold B1); `REGISTERED_LEAF_TAGS` extended 10→12; G4 LOAD-BEARING end-to-end define→lookup→inline-execute round-trip with byte-determinism splice falsifiability + store-identity invariant. Design FROZEN at `fb1b3ea` (R1.2 lite PASS 7.80/7.2 hard-mode + W3 honest-rescope). 2026-05-07. SHIPPED at `<merge-sha-tbd>` (T9.1 fills post-merge). | within 22-23 | `:skill/define` + `:skill/lookup` audit datoms emit on the canonical chain; SkillLibrary state round-trips Plan AST under coder control; ZERO `:skill/compose` datoms (procedural-recall scope, no dispatcher substitution). |
| **2.3c.2** | `:llm/call` recursion + `ComposeWithSkillAction` proposal acceptance — FD7 lift in `_searcher.py` (delete kind-string drop + isinstance belt-and-braces) + thread `SkillLibrary` through `_make_branch_expander`/`_make_branch_evaluator`/`mcts_search`; new `DispatcherContext` per coder iteration with 4-layer token enforcement (early reject + input estimation + provider-honest output cap + post-call hard accounting) + hard `MAX_LLM_CALL_DEPTH=3` + soft `MAX_RECURSIVE_TOKENS=20_000` + `MAX_RECURSIVE_REQUESTS=10`; dual-layer cycle detection (Layer A search-time STATIC at `_mcts.py:213-217` + Layer B execution-time DYNAMIC `cycle_path` stack with `SkillCycleDetected(_PlanCycleDetected)` subclass); `composed_skill_content_hash` field added to `:mcts/iteration` provenance for replay-explainability (NOT byte-identity, which comes from winner Plan AST content-addressing); `REGISTERED_LEAF_TAGS` extended 12→13 with `:llm/call`; G4 LOAD-BEARING end-to-end split into 5 sub-tests covering (a) chain integrity + (b) `request_count==2` cumulative + (c) peak depth=1 per call + (d) Layer A & Layer B cycle detection + (e) `composed_skill_content_hash == looked_up_plan.id` via full MCTS + (f) winner Plan AST replay byte-identity. **W3 RESCOPE at `98df051`**: LD5 `parent_audit_entry_id` field shipped at T2 plumbing layer (dataclass + chain-hash + wire form + datom round-trip + spec) but ACTIVE WIRING with non-None values RESCOPED to v0.9.x — the four-way constraint (G4 peak depth + LIFO finally + frozen AuditEntry + linear chain) cannot be simultaneously satisfied without ~150-200 LOC audit-chain redesign; G3 reformulated as STRUCTURAL-ONLY; G4 revised to SEQUENTIAL-recursion semantics. Design FROZEN at `3325199` (R1.4 lite PASS 7.68/7.2 hard-mode + W3 honest-rescope per CLAUDE.md 2.0d-at-6.4 precedent + 2.3c.1 R1.2 precedent). 2026-05-08. SHIPPED at `<merge-sha-tbd>` (T9.1 fills post-merge). | within 22-23 | `:llm/call` recursion audit-anchored under `DispatcherContext` ContextVar with budget enforcement; `ComposeWithSkillAction` proposals SURVIVE the expander wrapper (FD7 lifted) and reach `_apply_compose_with_skill` which grafts the skill body into the candidate plan; `:mcts/iteration` provenance datoms carry `composed_skill_content_hash` equal to `looked_up_plan.id` (32-hex content hash, NOT 22-char skill_id slice — length-discrimination falsifiability catches a "fake" skill_id-pinning impl); cycle detection rejects A→A self-graft via Layer A static check at the engine. |
