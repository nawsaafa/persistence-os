...
**Status:** Draft v0.5 — internal, not for external distribution until ARIS review round ≥ 2 passes.
...
The symbolic-planning and knowledge-representation literature deserves a citation here. LLM+PDDL integrations (Liu et al. 2023, *LLM+P*; Valmeekam et al. 2023, *PlanBench*) argue for routing LLMs through classical planners; Kambhampati and colleagues have repeatedly argued that LLMs-as-planners are unsound without a symbolic verifier. Persistence occupies neither pole: it is neither pure-LLM planning nor pure-PDDL verification, but a homoiconic AST in which leaf nodes may be LLM calls, symbolic code, or external planners — committing to the AST shape via `:persistence.plan/node` without committing to the leaf implementation. The symbolic substrate carries the structural guarantees (content-addressing, descendant propagation, spec conformance); the leaves stay free to be neural, procedural, or SMT-discharged.
...
**Hardening-track status honesty.** Three items shipped in v0.2.0a2 (commit `7bac436`) and are no longer future work: (i) a shared `persistence.spec.SpecError` base class so `PlanSpecError` composes with future sibling errors (R3-M2); (ii) a `:original-tag` escape hatch preserving aliased tags for MCTS/optimizer consumers that need the lossless-read path (R3-M3); (iii) an explicit schema-evolution contract documenting how ids change when new default-valued attrs are introduced on existing node kinds (R3-M5). Three items remain open and are Phase-2.C work: (i) the `fold()` primitive itself (R3-M1); (ii) a `Node.id` coercion registry for non-JSON-native types, e.g. `edn_format.Symbol` from reader-macro quotes (R3-M4) — design notes live in `docs/r3-m4-design/` pending a decision on whether to converge with the `effect.canonical` registry rather than ship a parallel one; (iii) substrate-wide 128-bit id-width consistency across Fact, Replay, and Plan. A separate v0.2.0a3 patch (commit `a9289ab`) closed a Prop-5 round-trip falsifier: hypothesis at `max_examples=200` generated a `Node` whose attr map included a user-supplied `id` key; the parser stripped it at parse but the constructor had accepted it, producing `parse(unparse(n)).id ≠ n.id` — reserved `id` / `:id` attr keys are now rejected at both construction and parse.
...
- **Artifact.** The Phase-1 + Phase-2.B artifact (`persistence-os @ v0.1.0a1 + v0.2.0a3`) is bundled with the paper submission. `pytest -q` from a clean clone runs the **783-passed / 7-xfailed** test suite in under one minute.
